...
namespace In_Lec
{
    class Camera
    {
        ...
        public void BuildNewSystem()
        {
            ...
            lookDir.X = lookAt.X - cop.X;//To find direction
            lookDir.Y = lookAt.Y - cop.Y;
            lookDir.Z = lookAt.Z - cop.Z;
            Matrix.Normalise(lookDir);//Normalise to be between 0 and 1

            basisa = Matrix.CrossProduct(up, lookDir);//Cross product to get new vector perpindicular to both
